...
termina_gera_geracao_peso_minimo_solucao(S):-write('Melhor Solucao: '),write(S),nl,!.

gera_geracao_peso_minimo_solucao([S*V|R],N,ValorMaxGeracoes,ValorMelhorSolucao):-
	write('Geracao '), write(N), write(':'), nl, write([S*V|R]), nl,
	(
		(	
			(
				(V < ValorMelhorSolucao) 
				; 
				(N =:= ValorMaxGeracoes)
			)
			,termina_gera_geracao_peso_minimo_solucao(S*V) 
		)
	)
	;
	gera_geracao_base([S*V|R],PopFinal),
	N1 is N + 1,
	gera_geracao_peso_minimo_solucao(PopFinal,N1,ValorMaxGeracoes,ValorMelhorSolucao).
